 ...
 * GitHub release + license badges and strips the
...
// Keep title + badges before Build; drop Build badge, logo, and ---- separator.
...
for (const name of ['preview-1-cursor.png', 'preview-2-cursor.png']) {
  const src = path.join(root, 'docs', 'assets', name)
  if (fs.existsSync(src)) {
    fs.copyFileSync(src, path.join(previewDestDir, name))
  }
}
const legacyPreview = path.join(previewDestDir, 'preview-cursor.png')
