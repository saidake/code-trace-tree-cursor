...
def xml_updated_at(path: Path) -> int:
    raw = xml_tag_text(path, "updatedAt")
    try:
        return int(raw)
    except ValueError:
        return 0
...
def sanitize_folder_name(project_root: Path) -> str:
...
def allocate_folder_named_xml(app_dir: Path, project_root: Path) -> Path:
    base = sanitize_folder_name(project_root)
...
def find_documents_by_path(project_root: Path) -> list[Path]:
...
def read_project_id(project_root: Path) -> str:
    ...
    best = max(matches, key=xml_updated_at)
    return xml_tag_text(best, "projectId")
...
def resolve_storage(project_root: Path) -> Path:
    ...
    if matches:
        if len(matches) == 1:
            return matches[0]
        return max(matches, key=xml_updated_at)
